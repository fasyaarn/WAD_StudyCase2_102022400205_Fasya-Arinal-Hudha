        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('samples.update', $sample) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')

                        <div>
                            <x-input-label for="kode_sampel" value="Kode Sampel" />
                            <x-text-input id="kode_sampel" name="kode_sampel" type="text" class="mt-1 block w-full disabled:bg-gray-100 disabled:text-gray-500" :value="old('kode_sampel', $sample->kode_sampel)" :disabled="! $isAdmin" />
                            <x-input-error :messages="$errors->get('kode_sampel')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="nama_sampel" value="Nama Sampel" />
                            <x-text-input id="nama_sampel" name="nama_sampel" type="text" class="mt-1 block w-full disabled:bg-gray-100 disabled:text-gray-500" :value="old('nama_sampel', $sample->nama_sampel)" :disabled="! $isAdmin" />
                            <x-input-error :messages="$errors->get('nama_sampel')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="jenis_sampel" value="Jenis Sampel" />
                            <select id="jenis_sampel" name="jenis_sampel" @disabled(! $isAdmin) class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm disabled:bg-gray-100 disabled:text-gray-500">
                        ...
                        <div>
                            ...
                            <x-text-input id="jumlah_titik" name="jumlah_titik" type="number" class="mt-1 block w-full disabled:bg-gray-100 disabled:text-gray-500" :value="old('jumlah_titik', $sample->jumlah_titik)" :disabled="! $isAdmin" />
                            <x-input-error :messages="$errors->get('jumlah_titik')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="biaya_per_titik" value="Biaya per Titik" />
                            <x-text-input id="biaya_per_titik" name="biaya_per_titik" type="number" class="mt-1 block w-full disabled:bg-gray-100 disabled:text-gray-500" :value="old('biaya_per_titik', $sample->biaya_per_titik)" :disabled="! $isAdmin" />
                        ...
                        <div>
                            ...
                            <textarea id="catatan_kondisi" name="catatan_kondisi" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('catatan_kondisi', $sample->catatan_kondisi) }}</textarea>
